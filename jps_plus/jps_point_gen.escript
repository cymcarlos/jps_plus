...
get_dis_jump_point_pre() ->
    Str =
        "%%%----------------------------------------------------------------------------
        %%%
        %%% @author chenyiming
        %%% @doc generated， do not  edit
        %%%----------------------------------------------------------------------------
        -module(sys_dis_jump_point).

        -include(\"jps.hrl\").
        \n".


main([ConfFile, SysDir]) ->
    BlockList = [{2, 0}, {6, 0},
        {6, 1},
        {1, 2}, {2, 2}, {6, 2}, {7, 2},
        {2, 3},
    ...
    %% todo 后面就根据ConfFile 解析了
    map_gen(BlockList, 4, 8),
    write(SysDir),
    ok.

write(SysDir) ->
    ok = filelib:ensure_dir(filename:nativename(SysDir) ++ "/"),
    StrPre = get_dis_jump_point_pre(),
    DisJumpPointList = ets:tab2list(?DISTANT_JUMP_POINT_ETS),
    F = fun(Rec, AccStr) ->
        {Row, Col} = Rec#dis_jump_point.key,
        Str1 =
            io_lib:format("get(~p, ~p) ->", [Row, Col])
            ++
            io_lib:format("#dis_jump_point{key = {~p, ~p}, down = ~p, down_right = ~p, right = ~p, up_right = ~p, up = ~p, up_left = ~p, left = ~p, down_left = ~p};\n",
                [Row, Col, Rec#dis_jump_point.down, Rec#dis_jump_point.down_left, Rec#dis_jump_point.right,
                    Rec#dis_jump_point.up_right, Rec#dis_jump_point.up, Rec#dis_jump_point.up_left, Rec#dis_jump_point.left, Rec#dis_jump_point.down_left]),
        AccStr ++ Str1
        end,
    Res = lists:foldl(F, StrPre, DisJumpPointList),
    ModName = "sys_dis_jump_point.erl",
    FileName = filename:join([SysDir, ModName]),
...
%↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
%% 测试例子
map_gen(BlockList, RowMax, ColMax) ->
%%    BlockList = [{2, 0}, {6, 0},
%%        {6, 1},
%%        {1, 2}, {2, 2}, {6, 2}, {7, 2},
%%        {2, 3},
...
calculate_map(IsWallFun, ValidFun, RowMax, ColMax) ->
    %% 初始化跳点ets
    ets:new(?JUMP_POINT_ETS, [name_table, set, public, {keypos, #jump_point.key}, {read_concurrency, true}]),
    RowList = lists:seq(0, RowMax),
    ColList = lists:seq(0, ColMax),
    %% 计算跳点
    calculate_jump_point_map(RowList, ColList, ValidFun, IsWallFun),
    %% 初始化跳点距离ets
    ets:new(?DISTANT_JUMP_POINT_ETS, [name_table, set, public, {keypos, #dis_jump_point.key}, {read_concurrency, true}]),
    %% 计算跳点距离
